...
function fnSetSajuName() {
	...
	$("#birth-info").text(smonth + " " + bean.birthday.replaceAll("-",".") + " " + bean.birthTime + "(" + "${ timePillar.stem2_k }" + "시) " + gender)
...
// 대운 셋팅
async function fnSetDaeun() {
    //1. 태어난 년도를 통해 순행/역행 판단한다.
    console.log(bean)
    let lunIljin = bean.lunIljin;
    let firstLun = lunIljin.substring(0,1);

    let value = tenganList.filter(v => v.cd === firstLun)[0]?.attr4;
    ...
    let twelveDays = await fnSetTwelveDays(bean.solYear, bean.solMonth);
    console.log(twelveDays)
}


//12절기 조회하기
function fnSetTwelveDays(year, month ) {
    var holidayArr = [];

    return new Promise((resolve, reject) => {
/*        var xhr = new XMLHttpRequest();
        var url = 'http://apis.data.go.kr/B090041/openapi/service/SpcdeInfoService/get24DivisionsInfo'; /!*URL*!/
        var queryParams = '?' + encodeURIComponent('serviceKey') + '=' +'Njev63C4hP6FAMTWCtsUYfhOsPQt42h2%2BfzDRfsobDdGO3T%2BGGeIXcjypeDrX3RdMF3YK8KnRuF22H8p44VLIw%3D%3D'; /!*Service Key*!/
        queryParams += '&' + encodeURIComponent('solYear') + '=' + encodeURIComponent(year);
        queryParams += '&' + encodeURIComponent('solMonth') + '=' + encodeURIComponent(month);
        xhr.open('GET', url + queryParams);
        ...
        queryParams += '&' + encodeURIComponent('solYear') + '=' + encodeURIComponent(year); /**/
        queryParams += '&' + encodeURIComponent('solMonth') + '=' + encodeURIComponent(month); /**/
        queryParams += '&' + encodeURIComponent('kst') + '=' + encodeURIComponent('0120'); /*한국 표준시각 */
        queryParams += '&' + encodeURIComponent('sunLongitude') + '=' + encodeURIComponent('285'); /*태양황경*/
        xhr.open('GET', url + queryParams);
        xhr.onreadystatechange = function () {
            if (this.readyState == 4) {
                console.log('Status: '+this.status+'nHeaders: '+JSON.stringify(this.getAllResponseHeaders())+'nBody: '+this.responseText);
